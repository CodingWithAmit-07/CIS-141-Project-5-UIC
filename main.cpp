...
//         bool move(int squareIndex, int destinationIndex);
//         void drop(int destinationIndex);
//         bool destroy();
...
void dfs(sf::Color positionColor,bool vis[],int dx[],int x, Square squaresArray[]){
    vis[x]=true;                                   
    for(int i=0;i<5;i++){                          
        int X=x+dx[i];                              
        
        sf::Color testColor = squaresArray[X].getColor();
        
        if(!vis[X] && testColor==positionColor){
            dfs(positionColor,vis,dx,X, squaresArray);
            squaresArray[X].setColor(BlankRGBValue, BlankRGBValue, BlankRGBValue);
        }                                          
     }                                              
}


void destroy(Square squaresArray[]){
    bool flag = false;
    
    for(int i =0; i < BoardColumns*BoardRows; i++){
        
        sf::Color positionColor = squaresArray[i].getColor();
        
        if((positionColor != BlankColor) && (positionColor != WallColor)){
                      
            sf::Color upColor = squaresArray[i+10].getColor();            
            sf::Color downColor = squaresArray[i-10].getColor();
            sf::Color leftColor = squaresArray[i-1].getColor();
            sf::Color rightColor = squaresArray[i+1].getColor();
            
            if((positionColor == leftColor) || (positionColor == rightColor) || (positionColor == upColor) || (positionColor == downColor)){
                int dx[]={0,1,-1,10,-10};                       
                bool vis[BoardRows*BoardColumns];             
                dfs(positionColor,vis,dx,i, squaresArray);    
...
void drop(int destinationIndex, Square squaresArray[ ]){
        
    int dropIndex = destinationIndex + BoardColumns;
    sf::Color dropColor = squaresArray[dropIndex].getColor();
    sf::Color destinationColor = squaresArray[destinationIndex].getColor();
    
    for(int i = BoardRows*BoardColumns; i>0; i--){
        
        while(dropColor == BlankColor){
            squaresArray[dropIndex].setColor(destinationColor);
            squaresArray[destinationIndex].setColor(dropColor);
            
            destinationIndex = dropIndex;
            dropIndex = destinationIndex + BoardColumns;
            
            dropColor = squaresArray[dropIndex].getColor();
            destinationColor = squaresArray[destinationIndex].getColor();
        }
        
        sf::Color dropColorHelper = squaresArray[i-10].getColor();
        sf::Color destinationColorHelper = squaresArray[i].getColor();
        
        if(destinationColorHelper == BlankColor && dropColorHelper != BlankColor && dropColorHelper != WallColor){
            squaresArray[i-10].setColor(destinationColorHelper);
            squaresArray[i].setColor(dropColorHelper);
        }
    }
   destroy(squaresArray); 
}


bool move(int squareIndex, int destinationIndex, Square squaresArray[ ]){
    ...
    if(destinationColor == BlankColor){       
        squaresArray[destinationIndex].setColor(sourceColor);    
        squaresArray[squareIndex].setColor(destinationColor);  
    }
    
}


int getNonEmptyTiles(Square squaresArray[]){                                         
    int notEmptyTiles = 0;                                 
    for(int i=0; i < BoardRows*BoardColumns; i++){
        
        sf::Color indexColor = squaresArray[i].getColor();
        
        if(indexColor != BlankColor && indexColor != WallColor){          
...
{	 
	...
	while (window.isOpen())
	{
		...
		// ...
		if(move(squareIndex, destinationIndex, squaresArray)){
            drop(destinationIndex, squaresArray);
            destroy(squaresArray);
            drop(destinationIndex, squaresArray);
            ...
            if(getNonEmptyTiles(squaresArray) == 0 && moveNumber <= allTheBoards.getParValue(currentBoardIndex)){ 
                std::cout<<"Congratulations!  On to the next level."<<std::endl;         
                score = score + moveNumber;//Giving the score of the player          
                ...
                std::cout<<std::endl;                                                    
                moveNumber = 0;                                                      
                }else if(getNonEmptyTiles(squaresArray) == 0 && moveNumber >= allTheBoards.getParValue(currentBoardIndex)){ //If we complete the game without meeting step requirements
                std::cout<<"Sorry, you took "<<moveNumber<<" moves and you must finish within "<<allTheBoards.getParValue(currentBoardIndex)<<" moves before moving on."<<std::endl;
                currentBoardIndex--;
